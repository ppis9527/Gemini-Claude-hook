...
if (require.main === module) {
    ...
    if (storeMode && instincts.length > 0) {
        ...
        // Trigger skill synthesis patch if new instincts were written
        try {
            const { execSync } = require('child_process');
            console.error('Triggering skill synthesis patch...');
            execSync(`node "${path.join(__dirname, 'synthesize-skills.js')}" --patch`, {
                encoding: 'utf8',
                timeout: 120000,
                stdio: ['pipe', 'pipe', 'pipe'],
            });
            console.error('Skill synthesis patch complete.');
        } catch (err) {
            console.error(`Skill synthesis patch failed (non-fatal): ${err.message?.slice(0, 100)}`);
        }
    }
}
