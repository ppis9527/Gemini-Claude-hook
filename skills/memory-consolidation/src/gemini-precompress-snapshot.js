...
function main() {
    ...
    // Extract structured checkpoint (non-blocking, best-effort)
    const checkpointScript = path.join(__dirname, 'extract-checkpoint.js');
    if (fs.existsSync(checkpointScript)) {
        try {
            const cpResult = spawnSync('node', [checkpointScript, sessionPath, '--agent', 'gemini'], {
                encoding: 'utf8',
                timeout: 60000,
                stdio: ['pipe', 'pipe', 'pipe']
            });
            if (cpResult.status === 0) {
                console.error('[PreCompress] Context checkpoint saved');
            }
        } catch {}
    }
