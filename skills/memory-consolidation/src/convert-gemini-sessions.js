...
// Markers found in fake sessions created by gemini -p extraction/instinct calls
const FAKE_SESSION_MARKERS = [
    'Extract persistent factual information',
    'You are analyzing',
    'write ONE concise behavioral instruction',
];
    ...
    for (const entry of fs.readdirSync(GEMINI_BASE)) {
        ...
        for (const file of fs.readdirSync(chatsDir)) {
            ...
            try {
                ...
                if (FAKE_SESSION_MARKERS.some(m => content.includes(m))) {
