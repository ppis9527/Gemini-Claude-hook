...
- **Dispatch Mode** (Claude Code): Headless task execution with automatic Telegram notifications
- **AskUserQuestion Forwarding**: Questions from headless Claude Code are sent to Telegram via PreToolUse hook
...
- Add Claude Code dispatch mode (`dispatch-claude-code.sh`)
- Add AskUserQuestion → Telegram notification via PreToolUse hook
- Document headless task execution workflow
